...
CustomLIULabel::CustomLIULabel(uint objectId, const QString &liuName, QWidget *parent)
    : QLabel(parent)
{
    if (objectName().isEmpty())
        setObjectName(QString("customLIULabel%1").arg(objectId));
    ...
    labelLIUName->setObjectName(QString("labelLIUName%1").arg(objectId));
    ...
    for (int i = 0; i < CardNumberPerLIU; i++) {
        ...
        labelCardSlotIndex->setObjectName(QString("labelCardSlotIndex%1").arg(objectId));
        labelCardSlotIndex->setAlignment(Qt::AlignCenter);
        contenstLayout->addWidget(labelCardSlotIndex, 0, i + 1, Qt::AlignCenter);

        auto liuCard = new CustomCardLabel(objectId, this);
